...
export const AthenaeumHeader: React.FC<AthenaeumHeaderProps> = ({ 
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-[#fcfbf7]/95 backdrop-blur-md border-b border-border-archival shadow-[0_1px_12px_rgba(40,30,15,0.05)] transition-all ${className}`}>
      <div className="h-20 w-full px-3 sm:px-5 lg:px-6 xl:px-8 flex items-center justify-between gap-2 sm:gap-4 max-w-[1600px] mx-auto">
        {/* Brand & Logo */}
        <div className="flex items-center gap-2.5 sm:gap-3 lg:gap-4 xl:gap-5 min-w-0">
          <Link to="/" className="flex items-center gap-2 sm:gap-2.5 group shrink-0 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 shadow-sm rounded-lg overflow-hidden border border-archival-teal/20 group-hover:scale-105 transition-transform bg-[#063b36] flex items-center justify-center">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-serif-display text-base sm:text-lg xl:text-xl font-bold tracking-tight text-archival-teal leading-none whitespace-nowrap shrink-0">
                  Athenaeum Library
                </span>
              ...
              <span className="text-[10px] sm:text-[11px] font-serif-body italic text-ink-muted mt-0.5 hidden 2xl:inline-block whitespace-nowrap">
                Editorial Archival &amp; Heritage Repositories
              </span>
              <span className="text-[10px] font-serif-body italic text-ink-muted mt-0.5 hidden xl:inline-block 2xl:hidden whitespace-nowrap">
                Heritage Archives
              </span>
            </div>
          </Link>
          ...
          {/* Desktop Nav */}
          <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5 min-w-0 shrink-0">
            <Link 
              to="/" 
              className={`px-2.5 xl:px-3 py-1.5 xl:py-2 font-sans-ui text-xs xl:text-sm font-semibold rounded-md transition-colors flex items-center gap-1.5 shrink-0 whitespace-nowrap ${
            ...
            >
              ...
              <span className="xl:hidden">Manuscripts &amp; Stacks</span>
            </Link>

            <Link 
              to="/user/borrowed" 
              className={`px-2.5 xl:px-3 py-1.5 xl:py-2 font-sans-ui text-xs xl:text-sm font-medium rounded-md transition-colors shrink-0 whitespace-nowrap ${
                isActive('/user') 
                  ? 'bg-archival-teal text-white' 
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-2.5 shrink-0">
          {showSearch && (
            <form onSubmit={handleSearchSubmit} className="relative hidden sm:flex items-center w-24 sm:w-28 md:w-36 lg:w-32 xl:w-40 2xl:w-52 focus-within:w-36 sm:focus-within:w-44 lg:focus-within:w-40 xl:focus-within:w-52 transition-all duration-200 shrink-0">
              <span className="material-symbols-outlined absolute left-2 sm:left-2.5 text-ink-muted/70 text-[16px] sm:text-[18px]">
                search
              </span>
              <input 
                type="text"
                ...
                placeholder="Search..."
                className="w-full bg-parchment-subtle border border-border-archival text-ink-primary pl-7 sm:pl-8 pr-7 sm:pr-8 py-1.5 rounded-md font-sans-ui text-xs placeholder:text-ink-muted/70 focus:outline-none focus:ring-1 focus:ring-archival-teal focus:bg-white truncate"
              />
              <div className="absolute right-1.5 hidden sm:flex items-center px-1.5 py-0.5 rounded bg-border-archival/60 text-ink-muted text-[10px] font-mono">
                ⌘K
              </div>
            </form>
          )}
          ...
          {isAuthenticated ? (
            <div className="relative shrink-0">
              ...
              >
                ...
                <span className="hidden sm:inline font-sans-ui text-xs font-semibold text-archival-teal max-w-[120px] lg:max-w-[150px] xl:max-w-[180px] truncate whitespace-nowrap">
                  {user?.name || 'Patron'}
                </span>
            ...
            <Link
              to="/login"
              className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg bg-archival-teal text-white hover:bg-archival-deep shadow-sm transition-all text-xs font-medium shrink-0 whitespace-nowrap"
            >
              <span className="material-symbols-outlined text-[16px] text-gilded-gold shrink-0">local_library</span>
              <span className="inline">Patron Sign-In</span>
            </Link>
          )}
